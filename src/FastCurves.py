...
def FastCurves(calculation,instru,time,name_planet,mag_star,band0,planet_spectrum,star_spectrum,tellurics,
               apodizer=None,strehl=None,coronagraph=None,plot_mag=True,separation_planet=None,mag_planet=None,channel=False,systematic=False,show_plot=True,print_value=True,
               post_processing="molecular mapping",sep_unit="arcsec",bkgd=None,star_pos="center",Rc=100,band_only=None,used_filter="gaussian"):
    """
    ...
    ----------
    calculation : str
        type of calculation wanted ("contrast" or "SNR")
    instru : str
    ...
    coronagraph : str, optional
        coronagrapg considered. The default is None.
    plot_mag : bool, optional
        for constrast curves in mag unit. The default is True.
    ...
    signal_band = [] ; DIT_band = [] ; sigma_s_2_band = [] ; sigma_ns_2_band = []
    ...
    syst_rv = star_spectrum.syst_rv ; delta_rv = planet_spectrum.delta_rv
    ...
    if show_plot: # on plot le flux stellaire de chaque band pour NIRCam
        f1 = plt.figure() ; band_flux = f1.add_subplot(111) ; band_flux.set_xlabel("wavelength (in µm)") ;  band_flux.set_ylabel("flux (in e-/mn)") ; band_flux.set_title(f"Star flux through coronagraph with {coronagraph} and mag({band0}) = {mag_star}")
    ...
    if calculation=="contrast" and show_contrast_comparison_all and show_plot : # comparaison des niveaux de bruit sur toutes les bandes
        fig, axs = plt.subplots(2,figsize=(6,6),sharex=True,sharey=True,layout="constrained",gridspec_kw={'wspace': 0.1,'hspace':0.1}) ; fig.suptitle(f'Comparison of noise levels in {name_planet} on-sky data from {instru}',fontsize=14) ; axs[0].set_title("with systematics (analytical) and with spatial variance (empirical)",fontsize=10, fontweight="bold") ; axs[1].set_title("without systematics (analytical) and with ERR extension (empirical)",fontsize=10, fontweight="bold") ; axs[1].set_xlabel(r'mean empirical std per spectral channel (in e-)',fontsize=10) ; axs[0].set_yscale("log") ; axs[1].set_yscale("log") ; axs[0].set_xscale("log") ; axs[0].grid(True) ; axs[1].grid(True) ; fig.text(-0.03, 0.5, 'mean analytical std per spectral channel (in e-)', va='center', rotation='vertical') ; cmap = get_cmap('Spectral', len(config_data['gratings']))
    ...
    for nb,band in enumerate(config_data['gratings']): # POUR CHAQUE BANDE
        ...
        if coronagraph is not None and instru=="NIRCam":
            peak_PSF = fits.getdata("sim_data/PSF/star_center/PSF_"+ instru + "/peak_PSF_" + band + "_" + coronagraph  + ".fits") # transmission du coronographe en fonction de la séparation de la cible
        ...
        if instru=="MIRIMRS":
            aper_corr = fits.getheader("sim_data/PSF/star_"+star_pos+"/PSF_"+instru+"/PSF_"+band+".fits")['AC']
        ...
        if post_processing=="molecular mapping" : # Molecular Mapping
            ...
            if systematic :
                ...
                if instru=="MIRIMRS" :
                    sigma_syst_prime_2,sigma_syst_2,sep,m_HF,Mp,wave = systematic_profile(config_data,band,sigma,Rc,R,star_spectrum_instru,planet_spectrum_instru,wave_inter,size_core,star_pos,used_filter) # en e-/Flux_tot
            ...
            # Calcul du alpha (nb de photons utiles /min au molecular mapping sur la bande considérée) (avec modulations systématique)
        ...
        if band==band_plot and calculation=="contrast" and show_contrast_comparison and not show_contrast_comparison_all and show_plot :
            plt.figure() ; plt.yscale("log") ;  plt.xlabel("wavelength (in µm)",fontsize = 14) ; plt.ylabel("Flux (in e-/mn)",fontsize = 14) ; plt.title('Star spectra on '+band+' (FastCurves VS data)',fontsize = 14)
            x,y= fits.getdata('utils/star_spectrum/star_spectrum_'+band+'_data.fits') ; y=y[(x>wave_inter[0]) & (x<wave_inter[-1])] ; x=x[(x>wave_inter[0]) & (x<wave_inter[-1])] ; plt.plot(x,y,'b') ; plt.ylim(np.nanmin(y),np.nanmax(y)) 
            plt.plot(x, star_spectrum_inter.interpolate_wavelength(star_spectrum_inter.flux*trans,star_spectrum_inter.wavelength,x, renorm = True).flux,'r') ; plt.legend(["data","FastCurves"])
        ...
        DIT , RON_eff = DIT_RON(instru,config_data,apodizer,PSF_profile,separation,star_spectrum_inter,time,min_DIT,max_DIT,trans,quantum_efficiency,RON,saturation_e,print_value)
        ...
        if bkgd is not None :
            ...
            bkgd_flux_inter *= 60 * DIT * np.nansum(bkgd_flux[1])/np.nansum(bkgd_flux_inter) # en e-/DIT/pixel + il faut renormaliser car on interpole 
        ...
        if post_processing == "molecular mapping":
            ...
            sigma_bkgd_2 = np.nansum(bkgd_flux_inter)*radial_transmission_interp # bruit/DIT  (radial_transmission_interp = transmission radiale)
        ...
        if calculation=="contrast":
            ...
            if show_plot : 
                if band==band_plot and not show_contrast_comparison_all : # noise contributions :
                    if post_processing=="molecular mapping" : # Molecular Mapping
                        if systematic :
                            plt.figure() ; plt.xlim(0,separation[-1]) ; plt.plot(separation,contrast,'k-') ; plt.plot(separation,5*np.sqrt(R_corr*size_core**2*(sigma_halo_prime_2)*(NDIT))/(signal*NDIT),'r--') ; plt.plot(separation,5*np.sqrt(R_corr*size_core**2*(sigma_ron_2)*(NDIT))/(signal*NDIT),'g--')  ; plt.plot(separation,5*np.sqrt(R_corr*size_core**2*(sigma_dc_2)*(NDIT))/(signal*NDIT),'m--') ;  plt.plot(separation,5*np.sqrt(R_corr*size_core**2*(sigma_bkgd_prime_2)*(NDIT))/(signal*NDIT),'b--') ; plt.plot(separation,5*np.sqrt(sigma_syst_prime_2)/(signal),'c--') ; plt.xlabel('separation (in arcsec)', fontsize = 14) ; plt.ylabel(r'contrast 5$\sigma_{CCF}$ / $\alpha_0$',fontsize = 14) ; plt.title(instru+f" noise contributions on {band}"+for_name_planet+"\n with "+"$t_{exp}$=" + str(round(time)) + "mn, $mag_*$("+band0+")=" + str(round(mag_star,2)) + f', $T_p$={T_planet}K and $R_c$ = {Rc}', fontsize = 14) ; plt.legend(["$\sigma_{CCF}$","$\sigma'_{halo}$","$\sigma_{ron}$","$\sigma_{dc}$","$\sigma'_{bkgd}$","$\sigma'_{syst}$"],fontsize=11) ; plt.yscale('log') ; plt.grid(True)# ; plt.ylim(1e-4,1e-1) ; plt.show()
                        else :
                            plt.figure() ; plt.xlim(0,separation[-1]) ; plt.plot(separation[separation>sep_min],contrast[separation>sep_min],'k-') ; plt.plot(separation[separation>sep_min],5*np.sqrt(R_corr*size_core**2*(sigma_halo_prime_2[separation>sep_min])*(NDIT))/(signal[separation>sep_min]*NDIT),'r--') ; plt.plot(separation[separation>sep_min],5*np.sqrt(R_corr*size_core**2*(sigma_ron_2)*(NDIT))/(signal[separation>sep_min]*NDIT),'g--') ; plt.plot(separation[separation>sep_min],5*np.sqrt(R_corr*size_core**2*(sigma_dc_2)*(NDIT))/(signal[separation>sep_min]*NDIT),'m--') ; plt.plot(separation[separation>sep_min],5*np.sqrt(R_corr*size_core**2*(sigma_bkgd_prime_2)*(NDIT))/(signal[separation>sep_min]*NDIT),'b--') ; plt.xlabel('separation (in mas)', fontsize = 14) ; plt.ylabel(r'contrast 5$\sigma$($\rho$) / $\alpha_0$',fontsize = 14) ; plt.title(instru+f" noise contributions on {band}"+for_name_planet+" \n (with "+"$t_{exp}$=" + str(round(time)) + "mn, $mag_*$("+band0+")=" + str(round(mag_star,2)) + f' and $T_p$={T_planet}K )', fontsize = 14) ; plt.legend(["$\sigma_{CCF}$","$\sigma_{halo}$","$\sigma_{RON}$","$\sigma_{DC}$","$\sigma_{bkgd}$"]) ; plt.yscale('log') ; plt.grid(True) ; plt.axvspan(0, sep_min, color='k', alpha=0.5, lw=0)
                        if show_contrast_comparison : # Pour comparer avec les données réelles 
                            plt.figure()
                            if systematic : 
                                plt.plot(separation,contrast,'r',label='$\sigma_{CCF}$ according to FastCurves (with systematics)')
                            plt.plot(separation,contrast_wo_syst,'r--',label='$\sigma_{CCF}$ according to FastCurves (without systematics)')
                            sep_data,sigma_spatial_2,sigma_err_2,_,_ = fits.getdata('utils/noiseMM/noiseMM_'+band+'.fits') ; f=interp1d(sep_data,5*np.sqrt(sigma_spatial_2*np.nansum(star_spectrum_inter.flux/DIT)**2), bounds_error=False, fill_value=np.nan) ; contrast_spatial=f(separation)/(signal*NDIT) ; f=interp1d(sep_data,5*np.sqrt(size_core**2*sigma_err_2*np.nansum(star_spectrum_inter.flux/DIT)**2), bounds_error=False, fill_value=np.nan) ; contrast_err=f(separation)/(signal*NDIT) ; contrast_err *= np.nanmean(contrast_wo_syst)/np.nanmean(contrast_err)
                            ...
                            plt.plot(separation,contrast_spatial,'b',label='$\sigma_{CCF}$ according to on-sky data (from spatial variance)') ; plt.plot(separation,contrast_err,'b--',label='$\sigma_{CCF}$ according to on-sky data (from ERR extension)')
                            plt.yscale('log') ; plt.xlabel('separation (in arcsec)', fontsize = 14) ; plt.ylabel(r'contrast 5$\sigma_{CCF}$ / $\alpha_0$',fontsize = 14) ; plt.title('FastCurves VS on-sky data'+f" on {band}"+for_name_planet+"\n with "+"$t_{exp}$=" + str(round(time)) + "mn, $mag_*$("+band0+")=" + str(round(mag_star,2)) + f', $T_p$={T_planet}K and $R_c$ = {Rc}', fontsize = 14) ; plt.grid(True) ; plt.legend(fontsize=11)# ; plt.ylim(1e-4,1e-1) ; plt.show()
                        if systematic : # plot du t_syst
                            ...
                            plt.figure() ; plt.plot(separation,t_syst,'b') ; plt.ylabel('$t_{syst}$ (in mn)',fontsize = 14) ; plt.xlabel('separation (in arcsec)',fontsize = 14) ; plt.grid(True) ; plt.title("$t_{syst}$"+f" on {band}"+"\n with $mag_*$("+band0+")=" + str(round(mag_star,2)) , fontsize = 14) ; plt.plot([separation[0],separation[-1]],[time,time],'r-') ; plt.yscale('log') ; plt.legend(["$t_{syst}$","$t_{exp}$ ="+f"{time} mn"]) 
                    elif post_processing=="ADI+RDI" : # RDI+ADI+RDI
                        plt.figure() ; plt.xlim(0,separation[-1]) ; plt.plot(separation,contrast,'k-') ; plt.plot(separation,5*np.sqrt(R_corr*size_core**2*(sigma_halo_2)*(NDIT))/(signal*NDIT),'r--') ; plt.plot(separation,5*np.sqrt(R_corr*size_core**2*(sigma_ron_2)*(NDIT))/(signal*NDIT),'g--')  ; plt.plot(separation,5*np.sqrt(R_corr*size_core**2*(sigma_dc_2)*(NDIT))/(signal*NDIT),'m--') ;  plt.plot(separation,5*np.sqrt(R_corr*size_core**2*(sigma_bkgd_2)*(NDIT))/(signal*NDIT),'b--') ; plt.xlabel('separation (in arcsec)', fontsize = 14) ; plt.ylabel(r'contrast (5$\sigma$ / $F_{max}$)',fontsize = 14) ; plt.title(instru+f" noise contributions on {band}"+for_name_planet+" \n with "+"$t_{exp}$=" + str(round(time)) + "mn and $mag_*$("+band0+")=" + str(round(mag_star,2)), fontsize = 14) ; plt.legend(["$\sigma_{tot}$","$\sigma_{halo}$","$\sigma_{RON}$","$\sigma_{DC}$","$\sigma_{bkgd}$","$\sigma_{syst}$"]) ; plt.yscale('log') ; plt.grid(True)
                elif show_contrast_comparison_all :
                    R_corr_px = np.zeros_like(separation)+1.
                    if instru=="MIRIMRS": # 4pt dithering
                        sep,r_corr_px = fits.getdata("sim_data/R_corr/star_"+star_pos+"/R_corr_MIRIMRS/R_corr_perpx_" + band + ".fits")
                        f = interp1d(sep,r_corr_px,bounds_error=False,fill_value='extrapolate')
                        R_corr_px = f(separation)
                    r_corr = np.zeros_like(sigma_halo_2)+1 ; r_corr_px = np.zeros_like(sigma_halo_2)+1
                    for i in range(r_corr.shape[1]):
                        r_corr[:,i] = R_corr ; r_corr_px[:,i] = R_corr_px
                    sigma_tot = np.sqrt(NDIT*r_corr*size_core**2*(sigma_halo_2 + sigma_ron_2 + sigma_dc_2 + sigma_bkgd_2 + sigma_syst_2*NDIT/(r_corr*size_core**2))) # bruit total en e- dans la boite intégré pendant le temps d'exposition
                    sigma_tot = np.nanmean(sigma_tot,1)
                    sigma_tot_wo_syst = np.sqrt(NDIT*r_corr_px*size_core**2*(sigma_halo_2 + sigma_ron_2 + sigma_dc_2 + sigma_bkgd_2)) # bruit total en e- dans la boite intégré pendant le temps d'exposition
                    sigma_tot_wo_syst = np.nanmean(sigma_tot_wo_syst,1)
                    sep_data,_,_,sigma_lambda_spatial_2,sigma_lambda_ERR_2 = fits.getdata('utils/noiseMM/noiseMM_'+band+'.fits') ; f=interp1d(sep_data,np.sqrt(sigma_lambda_spatial_2*np.nansum(star_spectrum_inter.flux/DIT)**2), bounds_error=False, fill_value=np.nan) ; sigma_lambda_spatial = f(separation) ; f=interp1d(sep_data,np.sqrt(size_core**2*sigma_lambda_ERR_2*np.nansum(star_spectrum_inter.flux/DIT)**2), bounds_error=False, fill_value=np.nan) ; sigma_lambda_ERR = f(separation) ; sigma_lambda_ERR *= np.nanmean(sigma_tot_wo_syst)/np.nanmean(sigma_lambda_ERR)
                    ...
                    axs[0].scatter(sigma_lambda_spatial,sigma_tot,label=band,c=cmap(nb),zorder=2) ; axs[1].scatter(sigma_lambda_ERR,sigma_tot_wo_syst,label=band,c=cmap(nb),zorder=2)
                    ...
                    if 1==0 :
                        points_syst = np.zeros((2,len(sigma_lambda_spatial))) ; points_syst[0] = sigma_tot ; points_syst[1] = sigma_lambda_spatial ; fits.writeto("utils/points/points_syst_"+band+"_"+name_planet+".fits",points_syst,overwrite=True)
                        points_wo_syst = np.zeros((2,len(sigma_lambda_spatial))) ; points_wo_syst[0] = sigma_tot_wo_syst ; points_wo_syst[1] = sigma_lambda_ERR ; fits.writeto("utils/points/points_wo_syst_"+band+"_"+name_planet+".fits",points_wo_syst,overwrite=True)
                        
                    if nb == len(config_data["gratings"])-1:
                        lims = [ np.min([axs[0].get_xlim(),axs[0].get_ylim()]),np.max([axs[0].get_xlim(),axs[0].get_ylim()]) ] ; axs[0].plot(lims, lims, 'k-') ; axs[1].plot(lims, lims, 'k-')  ; axs[1].legend()
        ...
        # Calcul du SNR
            ...
            if show_cos_theta_est and band==band_plot and calculation=="SNR" : # calcul du cos theta n (impact du bruit (et de l'auto-soustraction) sur l'estimation de la corrélation)
                ...
                plt.figure() ; plt.plot(separation,cos_theta_est,'k') ; plt.ylabel(r"cos $\theta_{est}$",fontsize=14) ; plt.grid(True) ; plt.title(f"Effect of noise and stellar substraction on the estimation of correlation between \n template and planetary spectrum for {instru} on {band} \n (assuming that the template is the same as the observed spectrum)")
                ...
                if separation_planet is not None and separation_planet < np.nanmax(separation) :
                    ...
                    plt.figure() ; plt.plot(cos_theta_est,cos_theta_p,'r') ; plt.grid(True) ; plt.ylabel(r"cos $\theta_p$",fontsize=14) ; plt.xlabel(r"cos $\theta_{est}$",fontsize=14) ; plt.title(f"Effect of noise and self-subtraction on correlation estimation \n {for_name_planet} on {band} with $R_c$ = {Rc}",fontsize=14)
    ...
    if calculation=="contrast" and show_plot:
        ...
        if separation_planet is not None and separation_planet < np.nanmax(separation) :
            if apodizer is not None :
                ...
                plt.plot([separation_planet,separation_planet],[np.nanmin(contrast_band[0]),np.nanmax(contrast_band[0])],'k--')
        if coronagraph is not None :
            plt.title(instru + " contrast curves"+for_name_planet+" with coronagraph \n (with $t_{exp}$=" + str(round(time,1)) + "mn and $mag_*$("+band0+")=" + str(mag_star) + f')', fontsize = 14)
        else:
            plt.title(instru + " contrast curves"+for_name_planet+f" with {post_processing}"+" \n (with $t_{exp}$=" + str(round(time,1)) + "mn, $mag_*$("+band0+")=" + str(round(mag_star,1)) + f' and $T_p$={round(T_planet)}K )', fontsize = 14)
        ...
        for i in range(len(contrast_band)):
            ax.plot(separation_band[i][separation_band[i]>sep_min], contrast_band[i][separation_band[i]>sep_min], label="Band " + name_band[i], color=cmap(i))
        ...
        ax.yaxis.set_ticks_position('both') ; plt.tight_layout() ; plt.show()
    ...
    # SNR :
        ...
        if separation_planet is not None :
            print('\n') ; print(f'MAX SNR (at {round(separation_planet,1)} {sep_unit}) = ',round(SNR_max_planet,1), "for "+name_max_SNR)
        ...
        if coronagraph is not None :
            plt.title(instru + " SNR curves"+for_name_planet+" with coronagraph \n (with $t_{exp}$=" + str(round(time)) + "mn, $mag_*$("+band0+")=" + str(round(mag_star,2)) + ", $mag_p$("+band0+")=" + str(round(mag_planet,2))+f' and $T_p$ = {T_planet}K)', fontsize = 14)
        else:
            plt.title(instru + " SNR curves"+for_name_planet+" with $t_{exp}$=" + str(round(time)) + "mn, \n $mag_*$("+band0+")=" + str(round(mag_star,2)) + ", $mag_p$("+band0+")=" + str(round(mag_planet,2))+f' and $T_p$ = {round(T_planet)}K ('+model+')', fontsize = 14)
        plt.xlabel(f"separation (in {sep_unit})", fontsize=14)
        ...
        ax.legend(loc='upper left') ; ax.set_xlim(0) ; ax.set_ylim(0)
        if separation_planet is not None  and separation_planet < np.nanmax(separation) :
            ax_legend = ax.twinx() ; ax_legend.plot([], [], '--', c='k', label=f'angular separation{for_name_planet}'); ax_legend.plot([], [], 'X', c='r', label='max SNR'+for_name_planet+' ('+str(round(SNR_max_planet,2))+')') ; ax_legend.legend(loc='lower right') ; ax_legend.tick_params(axis='y', colors='w')
...
def mirimrs(calculation,T_planet,lg_planet,mag_star,band0,T_star,lg_star,exposure_time,systematic=True,
            model="BT-Settl",mag_planet=None,separation_planet=None,name_planet=None,syst_radial_velocity=0,delta_radial_velocity=0,star_broadening=0,planet_broadening=0,channel=False,plot_mag=False,return_SNR_planet=False,return_quantity=False,show_plot=True,print_value=True,
            post_processing="molecular mapping",bkgd="medium",star_pos="center",star_spectrum=None,planet_spectrum=None,Rc=100,band_only=None):
    ...
    if planet_spectrum is None :
        if model == "Exo-REM":
            planet_spectrum = load_planet_spectrum(T_planet, lg_planet, model,version="old") #  Spectre de la planète : class Spectrum(wavel,flux,R,T) en J/s/m²/µm
        else :
            planet_spectrum = load_planet_spectrum(T_planet, lg_planet, model,version="new") #  Spectre de la planète : class Spectrum(wavel,flux,R,T) en J/s/m²/µm
    ...
    name_band,separation,curves,signal_band,sigma_s_2_band,sigma_ns_2_band,DIT_band=FastCurves(calculation,instrument,exposure_time,name_planet,mag_star,band0,planet_spectrum,star_spectrum,tellurics,
                                    apodizer=None,strehl=None,mag_planet=mag_planet,separation_planet=separation_planet,channel=channel,systematic=systematic,plot_mag=plot_mag,show_plot=show_plot,print_value=print_value,
                                    post_processing=post_processing,sep_unit=sep_unit,bkgd=bkgd,star_pos=star_pos,Rc=Rc,band_only=band_only)
...
def harmoni(calculation,T_planet,lg_planet,mag_star,band0,T_star,lg_star,exposure_time,systematic=False,
            apodizer="SP1",strehl="MED",model="BT-Settl",mag_planet=None,separation_planet=None,name_planet=None,syst_radial_velocity=0,delta_radial_velocity=0,star_broadening=0,planet_broadening=0,plot_mag=False,return_SNR_planet=False,return_quantity=False,show_plot=True,print_value=True,post_processing="molecular mapping",star_spectrum=None,planet_spectrum=None):
    warnings.filterwarnings('ignore', category=UserWarning, append=True)
    ...
    if star_spectrum is None :
        star_spectrum = load_star_spectrum(T_star,lg_star) # Spectre de l'étoile GNS93
    if planet_spectrum is None :
        planet_spectrum = load_planet_spectrum(T_planet, lg_planet, model) #  Spectre de la planète : class Spectrum(wavel,flux,R,T) en J/s/m²/µm
    if syst_radial_velocity != 0: # décalage Doppler (en km/s)
        star_spectrum = star_spectrum.doppler_shift(syst_radial_velocity)
        star_spectrum.syst_rv = syst_radial_velocity
    if delta_radial_velocity != 0 or syst_radial_velocity != 0 : # décalage Doppler (en km/s)
    ...
    name_band,separation,curves,signal_band,sigma_s_2_band,sigma_ns_2_band,DIT_band=FastCurves(calculation,instrument,exposure_time,name_planet,mag_star,band0,planet_spectrum,star_spectrum,tellurics,apodizer,strehl,
                                    mag_planet=mag_planet,separation_planet=separation_planet,plot_mag=plot_mag,show_plot=show_plot,print_value=print_value,
                                    post_processing=post_processing,sep_unit=sep_unit,bkgd=None)
    if return_SNR_planet : # POUR FASTYIELD
        if calculation != "SNR" :
            raise KeyError("THE CALCULATION NEED TO BE SET ON SNR !")
        if separation_planet is None :
...
def eris(calculation,T_planet,lg_planet,mag_star,band0,T_star,lg_star,exposure_time,systematic=False,
    ...
    name_band,separation,curves,signal_band,sigma_s_2_band,sigma_ns_2_band,DIT_band=FastCurves(calculation,instrument,exposure_time,name_planet,mag_star,band0,planet_spectrum,star_spectrum,tellurics,apodizer="NO_SP",strehl=strehl,
                                    mag_planet=mag_planet,separation_planet=separation_planet,plot_mag=plot_mag,show_plot=show_plot,print_value=print_value,
                                    post_processing=post_processing,sep_unit=sep_unit,bkgd=None)
    if return_SNR_planet : # POUR FASTYIELD
        if calculation != "SNR" :
...
def nircam(calculation,T_planet,lg_planet,mag_star,band0,T_star,lg_star,exposure_time,systematic=False,
           model="BT-Settl",mag_planet=None,separation_planet=None,name_planet=None,syst_radial_velocity=0,delta_radial_velocity=0,star_broadening=0,planet_broadening=0,plot_mag=False,return_SNR_planet=False,return_quantity=False,show_plot=True,print_value=True,post_processing="ADI+RDI",bkgd="medium",star_spectrum=None,planet_spectrum=None):
    warnings.filterwarnings('ignore', category=UserWarning, append=True)
    ...
    name_band,separation,curves,signal_band,sigma_s_2_band,sigma_ns_2_band,DIT_band=FastCurves(calculation,instrument,exposure_time,name_planet,mag_star,band0,planet_spectrum,star_spectrum,tellurics,
                                    coronagraph=coronagraph,mag_planet=mag_planet,separation_planet=separation_planet,plot_mag=plot_mag,show_plot=show_plot,print_value=print_value,
                                    post_processing=post_processing,sep_unit=sep_unit,bkgd=bkgd)
    if return_SNR_planet : # POUR FASTYIELD
        if calculation != "SNR" :
            raise KeyError("THE CALCULATION NEED TO BE SET ON SNR !")
        if separation_planet is None :
...
def nirspec(calculation,T_planet,lg_planet,mag_star,band0,T_star,lg_star,exposure_time,systematic=False,
            model="BT-Settl",mag_planet=None,separation_planet=None,name_planet=None,syst_radial_velocity=0,delta_radial_velocity=0,star_broadening=0,planet_broadening=0,plot_mag=False,return_SNR_planet=False,return_quantity=False,show_plot=True,print_value=True,post_processing="molecular mapping",bkgd="medium",star_spectrum=None,planet_spectrum=None):
    warnings.filterwarnings('ignore', category=UserWarning, append=True)
    ...
    tellurics = False # Effet de l'atmosphère (False)
    name_band,separation,curves,signal_band,sigma_s_2_band,sigma_ns_2_band,DIT_band=FastCurves(calculation,instrument,exposure_time,name_planet,mag_star,band0,planet_spectrum,star_spectrum,tellurics,
                                    apodizer=None,strehl=None,mag_planet=mag_planet,separation_planet=separation_planet,plot_mag=plot_mag,show_plot=show_plot,print_value=print_value,
                                    post_processing=post_processing,sep_unit=sep_unit,bkgd=bkgd)
    if return_SNR_planet : # POUR FASTYIELD
        if calculation != "SNR" :
            raise KeyError("THE CALCULATION NEED TO BE SET ON SNR !")
