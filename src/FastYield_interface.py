...
class MyWindow(tk.Tk): # https://koor.fr/Python/Tutoriel_Scipy_Stack/matplotlib_integration_ihm.wp
    ...
    def draw_table_plot(self):
        ...
        if self.systematics:
            if self.PCA:
                self.t_instru = load_planet_table(self.table+"_Pull_"+self.instru+"_"+self.apodizer+"_"+self.strehl+"_with_systematics+PCA_"+self.model+".ecsv")
            else:
        ...
        if self.instru == "ANDES":
            if self.band == "INSTRU":
                owa = self.config_data["FOV_fiber"]*0.010 * 1000 # en mas
            else:
                owa = self.config_data["FOV_fiber"]*self.config_data["pxscale"][self.band] * 1000 # en mas
        else:
