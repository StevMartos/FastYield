...
def get_template(wave, model, T, lg, vsini, rv, R, Rc, filter_type, epsilon=0.8, fastbroad=True, airmass=2.0, star_spectrum=None, wave_model=None):
    """
    ...
    if model in {"BT-NextGen", "Husser"}:
        ...
        template = load_planet_spectrum(T_planet=T, lg_planet=lg, model=model)
    ...
    # 5) To be homogenous to ph/bin or e-/bin or ADU/bin
...
def get_Ss_HF_LF(trans_Ss, trans, wave, R_nyquist, Rc, filter_type):
    ...
    Ss           = interp1d(wave[valid], Ss[valid], bounds_error=False, fill_value=np.nan)(wave) # Handle NaN
    Ss_HF, Ss_LF = filtered_flux(flux=Ss, R=R_nyquist, Rc=Rc, filter_type=filter_type)
    return Ss_HF, Ss_LF



def get_d_sim(d, wave, trans, model, T, lg, vsini, rv, R, Rc, filter_type, degrade_resolution, stellar_component, pca=None, trans_Ss=None, instru=None, epsilon=0.8, fastbroad=True, airmass=2.0, star_spectrum=None, cut_fringes=False, Rmin=None, Rmax=None, target_name=None, template=None, renorm_d_sim=True, wave_model=None, sigma_l=None, verbose=True):
    """"
    ...
    R_nyquist = get_resolution(wavelength=wave, func=np.nanmean)
    
    # 1) Template creation (or copy if injected)
    if template is None:
        template = get_template(instru=instru, wave=wave, model=model, T=T, lg=lg, vsini=vsini, rv=rv, R=R, Rc=Rc, filter_type=filter_type, epsilon=epsilon, fastbroad=fastbroad, airmass=airmass, star_spectrum=star_spectrum, wave_model=wave_model)
    else:
        template = template.copy()
    
    if verbose:
        print(f"\n get_d_sim: T={template.T:.0f}K, lg={template.lg:.2f}, rv={template.rv:.2f}km/s, vinsi={template.vsini:.2f}km/s...")
    
    # 2) Match instrument sampling
    if verbose:
        print(" get_d_sim: Degrading the resolution on 'wave'...")
    template = get_spectrum_band(spectrum_instru=template, wave_band=wave, R_output=R, degrade_resolution=degrade_resolution, verbose=verbose)

    # 3) HF/LF split of the planetary spectrum
    if verbose:
        print(" get_d_sim: HF/LF split...")
    template_HF, template_LF = filtered_flux(flux=template.flux, R=R_nyquist, Rc=Rc, filter_type=filter_type)
    ...
    if stellar_component and Rc is not None:
        ...
        d_sim       += - trans * Ss_HF * template_LF / Ss_LF
...
def get_S_res(wave, cube, Rc, filter_type, outliers=False, sigma_outliers=5, renorm_cube_res=False, only_high_pass=False, trans_Ss=None, debug=False):
    """
...
def get_CCF_2D_rv(instru, S_res, trans_Ss, T, lg, model, wave, trans, R, Rc, filter_type, rv_arr=None, vsini=None, pca=None, degrade_resolution=True, stellar_component=True, epsilon=0.8, fastbroad=True, airmass=2.0, star_spectrum=None, template_wo_shift=None):
    """
    ...
    if template_wo_shift is None:
        template_wo_shift = get_template(instru=instru, wave=wave, model=model, T=T, lg=lg, vsini=vsini, rv=0, R=R, Rc=Rc, filter_type=filter_type, epsilon=epsilon, fastbroad=fastbroad, airmass=airmass, star_spectrum=star_spectrum, wave_model=None)
    
    # Degrade template_wo_shift to target grid
    template = get_spectrum_band(spectrum_instru=template_wo_shift, wave_band=wave, R_output=R, degrade_resolution=degrade_resolution, verbose=False)
    ...
    template_HF.flux, template_LF.flux = filtered_flux(flux=template.flux, R=R_nyquist, Rc=Rc, filter_type=filter_type)
    ...
    for k in range(len(rv_arr)):
        ...
        if stellar_component and Rc is not None: # Adding the stellar component (if required)
            ...
            t                += - trans * Ss_HF * template_LF_shift / Ss_LF # it should be almost the same thing with or without the residual star flux
...
def get_CCF_1D_rv(instru, d, d_bkg, trans_Ss, wave, trans, T, lg, model, R, Rc, filter_type, large_rv=False, rv_arr=None, rv=None, vsini=None, pca=None, template_wo_shift=None, renorm_d_sim=True, epsilon=0.8, fastbroad=True, airmass=2.0, star_spectrum=None, calc_logL=False, method_logL="classic", sigma_l=None, weight=None, stellar_component=True, degrade_resolution=True, compare_data=False, cut_fringes=False, Rmin=None, Rmax=None, target_name=None, disable_tqdm=False, show=True, smooth_PSD=1, noise=None):
    ...
    if rv is not None:
        ...
        # Preparing the template
            ...
            d_sim = template_wo_shift.doppler_shift(rv)
            d_sim = get_d_sim(d=d/w, wave=wave, trans=trans, model=model, T=None, lg=None, vsini=None, rv=None, R=R, Rc=Rc, filter_type=filter_type, degrade_resolution=degrade_resolution, stellar_component=stellar_component, pca=pca, trans_Ss=trans_Ss, instru=instru, epsilon=epsilon, fastbroad=fastbroad, cut_fringes=cut_fringes, Rmin=Rmin, Rmax=Rmax, target_name=target_name, template=d_sim, renorm_d_sim=renorm_d_sim, wave_model=None, sigma_l=sigma_l, verbose=show)
        d_sim             *= w
        d_sim[d_sim == 0]  = np.nan
        d_sim[np.isnan(d)] = np.nan
        d[np.isnan(d_sim)] = np.nan
    ...
    if calc_logL:
        ...
        dpx_rv       = int(np.ceil(np.nanmax(np.abs(delta_lambda / dlambda))))
    else:
        dpx_rv = None
    ...
    global _RV_CTX
    _RV_CTX = dict(rv=rv, d=d, sigma_l=sigma_l, d_bkg=d_bkg, w=w, trans=trans, template_HF=template_HF, template_LF=template_LF, Ss_HF=Ss_HF, Ss_LF=Ss_LF, template_shift=template_shift, Rc=Rc, pca=pca, cut_fringes=cut_fringes, R=R, Rmin=Rmin, Rmax=Rmax, target_name=target_name, stellar_component=stellar_component, compare_data=compare_data, calc_logL=calc_logL, method_logL=method_logL, dpx_rv=dpx_rv)

    print()
    with Pool(processes=cpu_count()//2) as pool: 
        results = list(tqdm(pool.imap(process_correlation_rv, ((i, rv_arr[i]) for i in range(Nrv))), total=Nrv, desc=" get_CCF_1D_rv()", disable=disable_tqdm))
...
def process_correlation_rv(args):
    ...
    if calc_logL and sigma_l is not None:
        logL_value = get_logL(d[dpx_rv:-dpx_rv], t[dpx_rv:-dpx_rv], sigma_l[dpx_rv:-dpx_rv], method=method_logL)
    ...
    #plt.figure(dpi=300) ; plt.plot(np.isnan(d[dpx_rv:-dpx_rv])) ; plt.plot(np.isnan(t[dpx_rv:-dpx_rv])) ; plt.title(f"rv = {rv_i} km/S | N_d = {np.isfinite(d[dpx_rv:-dpx_rv]).sum()} | N_t = {np.isfinite(t[dpx_rv:-dpx_rv]).sum()}") ; plt.show()
...
def plot_CCF_1D_rv(instru, band, target_name, d, d_bkg, trans_Ss, wave, trans, T, lg, model, R, Rc, filter_type, large_rv=True, rv_arr=None, rv=None, vsini=None, pca=None, template_wo_shift=None, renorm_d_sim=True, epsilon=0.8, fastbroad=True, calc_logL=False, method_logL="classic", sigma_l=None, weight=None, stellar_component=True, degrade_resolution=True, compare_data=False, cut_fringes=False, Rmin=None, Rmax=None, verbose=True, show=True, smooth_PSD=1, rv_star=None, zoom_CCF=True, noise=None):
    """
...
def correlation_vsini(instru, d, d_bkg, trans_Ss, wave, trans, T, lg, model, R, Rc, filter_type, vsini_arr=None, rv=None, vsini=None, pca=None, template_wo_broad=None, epsilon=0.8, fastbroad=True, airmass=2.0, star_spectrum=None, calc_logL=False, method_logL="classic", sigma_l=None, weight=None, stellar_component=True, degrade_resolution=True, compare_data=False, cut_fringes=False, Rmin=None, Rmax=None, target_name=None, disable_tqdm=False):
    """
    ...
    # If not comparing data, calculating the raw template
        ...
        # Generate template if not provided
        if template_wo_broad is None:
            template_wo_broad = get_template(instru=instru, wave=wave, model=model, T=T, lg=lg, vsini=0, rv=rv, R=R, Rc=Rc, filter_type=filter_type, epsilon=epsilon, fastbroad=fastbroad, airmass=airmass, star_spectrum=star_spectrum, wave_model=None)
    ...
    if vsini is not None:
        ...
        # Preparing the template
            ...
            d_sim = get_d_sim(d=d/w, wave=wave, trans=trans, model=None, T=None, lg=None, vsini=None, rv=None, R=R, Rc=Rc, filter_type=filter_type, degrade_resolution=degrade_resolution, stellar_component=stellar_component, pca=pca, trans_Ss=trans_Ss, instru=instru, epsilon=epsilon, fastbroad=fastbroad, cut_fringes=cut_fringes, Rmin=Rmin, Rmax=Rmax, target_name=target_name, template=d_sim, renorm_d_sim=False, wave_model=None, sigma_l=sigma_l, verbose=True)
        d_sim             *= w
        d_sim[d_sim == 0]  = np.nan
        d_sim[np.isnan(d)] = np.nan
        d[np.isnan(d_sim)] = np.nan
    ...
    print()
    with Pool(processes=cpu_count()//2) as pool: 
        results = list(tqdm(pool.imap(process_correlation_vsini, ((i, vsini_arr[i]) for i in range(Nvsini))), total=Nvsini, desc=" correlation_vsini()", disable=disable_tqdm))
        for (i, CCF_value, corr_value, corr_auto_value, logL_value, CCF_bkg_value) in results:
            CCF[i]        = CCF_value
            corr[i]       = corr_value
            corr_auto[i]  = corr_auto_value
...
def plot_CCF_vsini(instru, band, target_name, d, d_bkg, trans_Ss, wave, trans, T, lg, model, R, Rc, filter_type, vsini_arr=None, rv=None, vsini=None, pca=None, template_wo_broad=None, epsilon=0.8, fastbroad=True, airmass=2.0, star_spectrum=None, calc_logL=False, method_logL="classic", sigma_l=None, weight=None, stellar_component=True, degrade_resolution=True, compare_data=False, cut_fringes=False, Rmin=None, Rmax=None, verbose=True, show=True):
    ...
    # Compute the radial velocity and cross-correlation functions for the data and background
    vsini_arr, CCF, corr, CCF_bkg, corr_auto, logL, sigma_CCF = correlation_vsini(instru=instru, d=d, d_bkg=d_bkg, trans_Ss=trans_Ss, wave=wave, trans=trans, T=T, lg=lg, model=model, R=R, Rc=Rc, filter_type=filter_type, vsini_arr=vsini_arr, rv=rv, vsini=vsini, pca=pca, template_wo_broad=template_wo_broad, epsilon=epsilon, fastbroad=fastbroad, airmass=airmass, star_spectrum=star_spectrum, calc_logL=calc_logL, method_logL=method_logL, sigma_l=sigma_l, weight=weight, stellar_component=stellar_component, degrade_resolution=degrade_resolution, compare_data=compare_data, cut_fringes=cut_fringes, Rmin=Rmin, Rmax=Rmax, target_name=target_name, disable_tqdm=not verbose)
...
def parameters_retrieval(instru, band, target_name, d, trans_Ss, wave, trans, model, R, Rc, filter_type, calc_logL=False, method_logL="classic", sigma_l=None, weight=None, pca=None, stellar_component=True, degrade_resolution=True, SNR_estimate=False, T_arr=None, lg_arr=None, vsini_arr=None, rv_arr=None, T=None, lg=None, vsini=None, rv=None, SNR_CCF=None, calc_d_sim=False, template=None, renorm_d_sim=True, fastcurves=False, Ss_HF=None, Ss_LF=None, wave_model=None, epsilon=0.8, fastbroad=True, airmass=2.0, star_spectrum=None, force_new_est=False, save=True, exposure_time=None, show=True, verbose=True):
    """
    ...
    trans_Ss : (N,) array
        Stellar flux on 'wave' (post-transmission); required if 'stellar_component=True'.
    trans : (N,) array
    ...
    stellar_component : bool
        Include residual stellar term in templates (requires Rc and trans_Ss).
    degrade_resolution : bool
    ...
    # Compute new estimations if files are missing or forced update
        ...
        if calc_d_sim and calc_logL:
            d_sim              = get_d_sim(d=d/w, wave=wave, trans=trans, model=model, T=T, lg=lg, vsini=vsini, rv=rv, R=R, Rc=Rc, filter_type=filter_type, degrade_resolution=degrade_resolution, stellar_component=stellar_component, pca=pca, trans_Ss=trans_Ss, instru=instru, epsilon=epsilon, fastbroad=fastbroad, template=template, renorm_d_sim=renorm_d_sim, wave_model=wave_model, sigma_l=sigma_l, verbose=verbose)
            d_sim             *= w
            d_sim[d_sim == 0]  = np.nan
            d_sim[np.isnan(d)] = np.nan
            d[np.isnan(d_sim)] = np.nan
        ...
        global _TLG_CTX
        _TLG_CTX    = dict(R_nyquist=R_nyquist, vsini_arr=vsini_arr, rv_arr=rv_arr, d=d, w=w, pca=pca, model=model, wave=wave, trans=trans, epsilon=epsilon, fastbroad=fastbroad, airmass=airmass, star_spectrum=star_spectrum, R=R, Rc=Rc, filter_type=filter_type, sigma_l=sigma_l, calc_logL=calc_logL, method_logL=method_logL, dpx_rv=dpx_rv, Ss_HF=Ss_HF, Ss_LF=Ss_LF, SNR_estimate=SNR_estimate, rv=rv, stellar_component=stellar_component, degrade_resolution=degrade_resolution, d_sim=d_sim, wave_model=wave_model)
        NT          = len(T_arr)
        Nlg         = len(lg_arr)
        Nvsini      = len(vsini_arr)
        Nrv         = len(rv_arr)
...
def process_parameters_estimation(args):
    ...
    dpx_rv             = _TLG_CTX['dpx_rv']
    Ss_HF              = _TLG_CTX['Ss_HF']
    Ss_LF              = _TLG_CTX['Ss_LF']
    ...
    if Nvsini == 1: # more accurate but slower (it's ok because Nvsini == 1)
        template = get_template(wave=wave, model=model, T=T, lg=lg, vsini=vsini_arr[0], rv=0, R=R, Rc=Rc, filter_type=filter_type, epsilon=epsilon, fastbroad=fastbroad, airmass=airmass, star_spectrum=star_spectrum, wave_model=wave_model)
    else: # less accurate but faster
        template = get_template(wave=wave, model=model, T=T, lg=lg, vsini=0, rv=0, R=R, Rc=Rc, filter_type=filter_type, epsilon=epsilon, fastbroad=fastbroad, airmass=airmass, star_spectrum=star_spectrum, wave_model=wave_model)
    ...
    template = get_spectrum_band(spectrum_instru=template, wave_band=wave, R_output=R, degrade_resolution=degrade_resolution)
    ...
    for k in range(Nvsini):
        ...
        if SNR_estimate: # Reference for sigma_auto_correlation (and subtract it later)
            ...
            if stellar_component and Rc is not None:
                template_auto = template_auto - trans * Ss_HF * template_LF_broad.flux/Ss_LF # better without the residual stellar contributions for the auto-correlation
        ...
        for l in range(Nrv):
            ...
            if stellar_component and Rc is not None:
                ...
                t                      += - trans * Ss_HF * template_LF_broad_shift/Ss_LF
            ...
            if SNR_estimate:
                ...
                if calc_logL:
                    logL_2D[k, l] = get_logL(d[dpx_rv:-dpx_rv], t[dpx_rv:-dpx_rv], sigma_l[dpx_rv:-dpx_rv], method=method_logL)
                    if d_sim is not None:
                        logL_2D_sim[k, l] = get_logL(d_sim[dpx_rv:-dpx_rv], t[dpx_rv:-dpx_rv], sigma_l[dpx_rv:-dpx_rv], method=method_logL)
