...
def stellar_high_filtering(c,renorm_cube_res,R,Rc,used_filter,cosmic=False,sigma_cosmic=5,print_value=True,only_high_pass=False):
    cube = np.copy(c)
    NbChannel, NbLine, NbColumn = cube.shape
    stell = np.nansum(cube,(1,2)) # spectre stellaire estimé
    #stell = np.nansum(cube*annular_mask(0,max(NbLine,NbColumn)//8,size=(NbLine, NbColumn)),(1,2)) # spectre stellaire estimé
...
def molecular_mapping_rv(instru,cube_high_filtered,T,lg,model,wave,trans,R,Rc,used_filter,rv=None,vsini=0,print_value=True,planet_spectrum=None,pca=None):
        ...
        for i in range(NbLine):
            for j in range(NbColumn): # pour chaque spaxel
                if not all(np.isnan(cube_high_filtered[:,i,j])): # on ignore les NaN
                    d = np.copy(cube_high_filtered[:,i,j])
                    t = np.copy(template)
                    d[d==0] = np.nan
                    t[np.isnan(d)] = np.nan
                    t[t==0] = np.nan
                    d[np.isnan(t)] = np.nan
                    t /= np.sqrt(np.nansum(t**2)) # on normalise le template à 1
...
def correlation_rv(instru,sd_HF,wave,trans,T,lg,model,R,Rc,used_filter,show=True,large_rv=False,vsini=0,rv=None,pca=None,correlation=True,noise_estimation=False):
    planet_spectrum = load_planet_spectrum(T,lg,model=model,instru=instru)
    if model[:4] == "mol_" and Rc is not None:
        _,planet_spectrum_BF = filtered_flux(planet_spectrum.flux,R=planet_spectrum.R,Rc=Rc,used_filter=used_filter)
        sg = sigma_clip(planet_spectrum_BF,sigma=1)
...
def plot_CCF(instru,sd_HF,bs_HF,wave,trans,T,lg,model,R,Rc,used_filter,target_name,band,rv0,vsini=0,large_rv=True,pca=None): 
    plt.figure(dpi=300) ; plt.xlabel("radial velocity (in km/s)",fontsize=14) ; plt.ylabel(r"cos $\theta_{est}$",fontsize=14) ; plt.grid(True)
    cos_signal , rv = correlation_rv(instru,sd_HF,wave,trans,T=T,lg=lg,model=model,R=R,Rc=Rc,used_filter=used_filter,show=False,large_rv=large_rv,vsini=vsini,pca=pca)
    ...
    # Sp_HF *= trans 
    # Sp_HF[np.isnan(sd_HF)] = np.nan ; Sp_HF[Sp_HF==0] = np.nan
...
def SNR_calculation(CCF,CCF_wo_planet,y0,x0,size_core,print_value=True,snr_calc=True):
        ...
        if planet_sep == 0 : 
            ...
            CCF_noise = np.copy(CCF)*annular_mask(3*size_core,4*size_core,value=np.nan,size=CCF.shape)
        else :
            CCF = CCF*annular_mask(max(planet_sep-3*size_core-1,0),planet_sep+3*size_core,value=np.nan,size=CCF.shape)
            if snr_calc :
                CCF_noise = np.copy(CCF_wo_planet)*annular_mask(max(1,planet_sep-size_core-1),planet_sep+size_core,value=np.nan,size=CCF_wo_planet.shape)
            else :
...
def correlation_T_rv(instru,sd_HF,wave,trans,R,Rc,used_filter,target_name,band,model="BT-Settl",vsini=0,pca=None):
    ...
    for i in range(len(lg)):
        ...
        for j in range(len(T)):
            ...
            cos_rv,rv = correlation_rv(instru=instru,sd_HF=sd_HF,wave=wave,trans=trans,T=T[j],lg=lg[i],model=model,R=R,Rc=Rc,used_filter=used_filter,show=False,large_rv=False,vsini=vsini,rv=rv,pca=pca)
            cos_3d[i,j,:] = cos_rv
            k+=1
    ...
    correlation_rv(instru=instru,sd_HF=sd_HF,wave=wave,trans=trans,T=T[idx_max_corr[1]],lg=lg[idx_max_corr[0]],model=model,R=R,Rc=Rc,used_filter=used_filter,show=True,large_rv=False,vsini=vsini,rv=rv,pca=pca)
...
def SNR_T_rv(instru,Sres,Sres_wo_planet,x0,y0,size_core,sd_HF,rv,wave,trans,R,Rc,used_filter,target_name,band,model="BT-Settl",vsini=0,pca=None):
    T,lg = model_T_lg_array(model)
    ...
    correlation_rv(instru=instru,sd_HF=sd_HF,wave=wave,trans=trans,T=T[idx_max_snr[1]],lg=lg[idx_max_snr[0]],model=model,R=R,Rc=Rc,used_filter=used_filter,show=True,large_rv=False,vsini=vsini,pca=pca)
...
def chi2_rv(instru,sd_HF,wave,trans,T,lg,R,Rc,used_filter,sigma_l,model="BT-Settl",show=True,large_rv=False,vsini=0):
    config_data = get_config_data(instru)
    planet_spectrum = load_planet_spectrum(T,lg,model=model,instru=instru)
    ...
    for i in range(len(rv)) :
        ...
        R = np.nansum(sd_HF*template/sigma_l**2) / np.nansum(template**2/sigma_l**2) # voir Malin et al. 2023
...
def chi2_T_rv(instru,sd_HF,wave,trans,R,Rc,used_filter,sigma_l,target_name,band,model="BT-Settl",vsini=0):
    T,lg = model_T_lg_array(model)
    ...
    for i in range(len(lg)):
        for j in range(len(T)):
            print(round(100*(k+1)/(len(T)*len(lg)),2),"%")
            chi2,rv = chi2_rv(instru,sd_HF,wave,trans,T=T[j],lg=lg[i],R=R,Rc=Rc,used_filter=used_filter,sigma_l=sigma_l,model=model,show=False,large_rv=False,vsini=vsini)
