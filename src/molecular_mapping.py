...
def flux_shift(wave, flux, rv):
    ...
    wshift = wave_hr * (1 + (
                rv / const.c)).value  # const.c = vitesse de la lumière (via astropy) / axe de longueur d'onde décalé
    f = interp1d(wave, flux, bounds_error=False,
                 fill_value=np.nan)  # créer une interpolation avec l'axe lambda décalé / le spectre décalé
    flux = f(wave_hr)  # interpole les valeurs du flux sur le nouvel axe (wavelength_output)
    f = interp1d(wshift, flux, bounds_error=False,
                 fill_value=np.nan, )  # créer une interpolation avec l'axe lambda décalé / le spectre décalé
...
def qqplot(map_w):
    ...
    plt.figure()
    sm.qqplot(list_cn_w, line='45')
    plt.xlim(-5, 5)
    plt.ylim(-5, 5)
    plt.title('Q-Q plots of the CCF', fontsize=14)
...
def qqplot2(map1, map2):
    ...
    plt.legend()
    plt.title('Q-Q plots of the CCF of CT Cha b on 1SHORT', fontsize=14)
    plt.ylabel("sample quantiles", fontsize=14)
    plt.xlabel("theoritical quantiles", fontsize=14)
    plt.grid(True)
    plt.show()
...
def open_data(instru, target_name, band, R, crop_band=False, cosmic=False, sigma_cosmic=5):
    ...
    wave = (np.arange(hdr['NAXIS3']) + hdr['CRPIX3'] - 1) * hdr['CDELT3'] + hdr[
        'CRVAL3']  # axe de longueur d'onde des données en µm
    area = config_data['telescope']['area']  # aire collectrice m²
    S *= pxsteradian * 1e6  #pxsteradian * 1e6 * 1e-26 * step * 1e-6 * wave[i] * 1e-6 / (h * c) * area * exposure_time
    err *= pxsteradian * 1e6  # MJy/Steradian => Jy/px
    S *= 1e-26
    err *= 1e-26  # Jy/pixel => J/s/m²/Hz/px
    for i in range(S.shape[0]):
        S[i] *= c / ((wave[i] * 1e-6) ** 2)
        err[i] *= c / ((wave[i] * 1e-6) ** 2)  # J/s/m²/Hz/px => J/s/m²/m/px
    S *= step * 1e-6
    err *= step * 1e-6  # J/s/m²/m/px => J/s/m²/px
    for i in range(S.shape[0]):
        S[i] *= wave[i] * 1e-6 / (h * c)
        err[i] *= wave[i] * 1e-6 / (h * c)  # J/s/m²/px => ph/s/m²/px
    S *= area
    err *= area  # ph/s/m²/px => photons/s/px
    if instru == "MIRIMRS":
        if crop_band:
            ...
            wave_trans, trans = fits.getdata(
                "sim_data/Transmission/MIRIMRS/Instrumental_transmission/transmission_" + band + ".fits")
        ...
        wave = wave[(wave > config_data['gratings'][band].lmin) & (wave < config_data['gratings'][band].lmax)]
        wave_trans, trans = fits.getdata(
            "sim_data/Transmission/NIRSpec/Instrumental_transmission/transmission_" + band + ".fits")
        f = interp1d(wave_trans, trans, bounds_error=False, fill_value=0)
        trans = f(wave)
        for i in range(S.shape[0]):
            S[i] *= trans[i]
            err[i] *= trans[i]  # e-/s/pixel
    S *= exposure_time
    err *= exposure_time  # e-/pixel or ph/pixel
    cube = crop(S)
    err = crop(err)  # on met l'étoile au centre du cube
    cube[cube == 0] = np.nan
    err[err == 0] = np.nan
    if cosmic:
    ...
    plt.figure()
    plt.plot(wave, star_flux, 'b')
    plt.xlabel("wavelength (in µm)")
    plt.ylabel("flux (in e-/mn)")
    plt.title(f"Star flux estimated from data on {band} of {instru} for {target_name}")
    plt.grid(True)
    plt.show()
    B = np.zeros((2, np.size(wave)))
    B[0, :] = wave
    B[1, :] = star_flux
    ...
    print(' target name =', target_name, " / pixelscale =", round(pxscale, 3), ' "/px & pxsteradian =', pxsteradian,
          "Sr/px")
    if target_name != "HR8799" or target_name != "GJ504" or target_name != "Bpic":
        print(" DATE :", hdr0["DATE-OBS"])
    dl = wave - np.roll(wave, 1)
    dl[0] = dl[1]  # array de delta Lambda
    R = np.nanmean(wave / (2 * dl))  # calcule de la nouvelle résolution
    print(' R = ', R)
    return cube, wave, pxscale, err, trans, target_name, exposure_time


def stellar_high_filtering(c, renorm_cube_res, R, Rc, used_filter, cosmic=False, sigma_cosmic=5,
                           print_value=True):
    """
    
    Parameters
    ----------
    c (numpy array, (NbLambda x NbLine x NbColumn)): non-filtered data cube
    renorm_cube_res (bool): renormalize the output data cube if True 
    R (float): Spectral resolution
    Rc (float): Cut-off resolution 
    used_filter (str): Filtering method ("gaussian", "smoothstep", "step")
    cosmic (optional, bool): Using a filtering method to attenuate cosmic if True
    sigma_cosmic (optional, int): Kernel size for cosmic filtering
    print_value (optional, bool): print xxx #TODO

    Returns
    -------
    cube_res (numpy array): high-pass filtered cube
    cube_M (numpy array): PSF cube
    """
    cube = np.copy(c)
...
def molecular_mapping_rv(instru, band, cube_high_filtered, T, lg, model, wave, trans, calculation, R, Rc, used_filter,
                         broad=0, rv=0, print_value=True):
    """
    Function to perform molecular mapping on a non-filtered cube
    Parameters
    ----------
    instru (str): instrument name (see config.py)
    band (str): wavelength band used (see config.py)
    cube_high_filtered (numpy array NbLambda x NbColumn x NbLine): high-pass filtered data cube
    T (int): Temperature of the planet
    lg (float): log(g) of the planet
    model (str): template librairy ("BT-Settl", "BT-Dusty", "Exo-REM", "PICASO", "Morley", "Saumon", "SONORA")
    wave (numpy array): wavelength axis of the data in µm
    trans (numpy array): global transmission
    calculation (str): TODO
    R (int): Resolution of the instrument
    Rc (int): Cut-off resolution for high-pass filter
    used_filter (str): Filtering method ("gaussian", "smoothstep", "step")
    broad (float): rotational broadening in km/s
    rv (float): doppler shift in km/s
    print_value (optional, bool) : return the CCF cube, the radial velocity axis and the high-pass filtered normalized templated use for correlation

    Returns
    -------
    CCF (numpy array): Correlation map cube
    rv (numpy 1D array): radial velocity axis
    t (numpy 1D array): high-pass filtered normalized templated use for correlation
    ...
    for k in range(len(rv)):
        ...
        planet_shift.flux = planet_shift.flux[valid]
        planet_shift.wavelength = planet_shift.wavelength[valid]
        template, _ = filtered_flux(planet_shift.flux, R, Rc, used_filter)
        f = interp1d(planet_shift.wavelength, template, bounds_error=False,
                     fill_value=np.nan)  # créer une interpolation avec l'axe lambda décalé / le spectre décalé
...
def correlation_rv(instru, sd_HF, wave, trans, T, lg, R, Rc, used_filter, model="BT-Settl", show=True, large_rv=False,
                   broad=0):
    config_data = get_config_data(instru)
    if instru == "MIRIMRS" and model == "Exo-REM":
        planet_spectrum = load_planet_spectrum(T, lg, model=model, version="old")
    ...
    for i in range(len(rv)):
        ...
        f = interp1d(planet_shift.wavelength, Sp_HF, bounds_error=False,
                     fill_value=np.nan)  # créer une interpolation avec l'axe lambda décalé / le spectre décalé
    ...
    if show:
        plt.figure()
        plt.plot(wave, Sp_HF / np.sqrt(np.nansum(Sp_HF ** 2)), 'b', label="template")
        plt.plot(wave, S_res / np.sqrt(np.nansum(S_res ** 2)), 'r', label="data")
        plt.xlabel("wavelength (in µm)")
        plt.ylabel("high-pass flux (normalized")
        plt.legend()
        plt.show()
    return cos_theta, rv


def correlation_rv_corr(instru, sd_HF, wave, wave_hr, trans_hr, T, lg, sigma, model="BT-Settl", show=True,
                        large_rv=False, broad=0):
    config_data = get_config_data(instru)
    if instru == "MIRIMRS" and model == "Exo-REM":
        planet_spectrum = load_planet_spectrum(T, lg, model=model, version="old")
    else:
    ...
    if show:
        plt.figure()
        plt.plot(wave, template, 'b', label="template")
        plt.plot(wave, template_data, 'r', label="data")
        plt.xlabel("wavelength (in µm)")
        plt.ylabel("high-pass flux (normalized")
        plt.legend()
        plt.show()
            ...
            if not np.isnan(CCF[i_shift, j_shift]):
                for i in range(PSF_shift.shape[0]):
                    for j in range(PSF_shift.shape[1]):
                        if i + i_PSF_centroid - i_shift >= 0 and j + j_PSF_centroid - j_shift >= 0 and i + i_PSF_centroid - i_shift < \
                                PSF_shift.shape[0] and j + j_PSF_centroid - j_shift < PSF_shift.shape[1]:
...
def plot_CCF(instru, sd_HF, bs_HF, wave, trans, R, Rc, used_filter, target_name, band, model="BT-Settl", broad=0,
             large_rv=True):
    T0 = np.array([500, 1500, 2500])
    lg = 3.5
    plt.figure()
    plt.xlabel("Doppler velocity (in km/s)", fontsize=14)
    plt.ylabel(r"cos $\theta_{est}$", fontsize=14)
    plt.grid(True)
    for i in range(len(T0)):
        cos, rv = correlation_rv(instru, sd_HF, wave, trans, T=T0[i], lg=lg, R=R, Rc=Rc, used_filter=used_filter,
                                 model=model, show=False, large_rv=large_rv, broad=broad)
        plt.plot(rv, cos, label=f"T = {T0[i]} K")
        if i == int(len(T0) - 1):
            cos, rv = correlation_rv(instru, bs_HF, wave, trans, T=T0[i], lg=lg, R=R, Rc=Rc, used_filter=used_filter,
                                     model=model, show=False, large_rv=large_rv, broad=broad)
            plt.plot(rv, cos, "k", label=f"off-planet")
    plt.legend(fontsize=12)
    plt.title(
        f'Correlation between {model} spectra and {target_name} data spectrum \n on {band} of {instru} with $R_c$ = {Rc}',
        fontsize=16)
...
def correlation_T_rv(instru, sd_HF, wave, trans, R, Rc, used_filter, target_name, band, model="BT-Settl", broad=0):
    ...
    for i in range(len(lg)):
        for j in range(len(T)):
            ...
            cos_rv, rv = correlation_rv(instru, sd_HF, wave, trans, T=T[j], lg=lg[i], R=R, Rc=Rc,
                                        used_filter=used_filter, model=model, show=False, large_rv=False, broad=broad)
    ...
    print(
        f"maximum correlation value of {round(np.nanmax(cos_2d), 2)} for T = {T[idx_max_corr[1]]} K, lg = {lg[idx_max_corr[0]]} rv = {rv_2d[idx_max_corr[0], idx_max_corr[1]]} km/s")
    plt.figure()
    plt.pcolormesh(T, lg, cos_2d, cmap=plt.get_cmap('rainbow'), vmin=np.nanmin(cos_2d), vmax=np.nanmax(cos_2d))
    plt.xlabel("planet's temperature (in K)", fontsize=12)
    plt.ylabel("planet's gravity surface", fontsize=12)
    plt.title(
        f'Correlation between {model} spectra and {target_name} \n data spectrum on {band} of {instru} with $R_c$ = {Rc}',
        fontsize=14)
    cbar = plt.colorbar()
    cbar.set_label(r"cos $\theta_{est}$", fontsize=14, labelpad=20, rotation=270)
    plt.plot([T[idx_max_corr[1]], T[idx_max_corr[1]]], [lg[idx_max_corr[0]], lg[idx_max_corr[0]]], 'kX', ms=10,
             label=r"cos $\theta_{max}$ = " + f"{round(np.nanmax(cos_2d), 2)} for T = {T[idx_max_corr[1]]} K, \n lg = {lg[idx_max_corr[0]]} and rv = {round(rv_2d[idx_max_corr[0], idx_max_corr[1]], 2)} km/s")
...
def SNR_T_rv(instru, cube_high_filtered, wave, trans, R, Rc, used_filter, target_name, band, model, broad, y0, x0,
             size_core):
    if target_name == "Chamaeleon":  # on masque les bords
        cube_high_filtered[:, :cube_high_filtered.shape[1] // 2, :] = np.nan
        cube_high_filtered[:, :, :cube_high_filtered.shape[1] // 2] = np.nan  # on masque les bords
    elif target_name == "HR8799":  # on masque les autres planètes et les bords
        cube_high_filtered[:, 34:, :] = np.nan
        cube_high_filtered[:, 27 - 4:27 + 5, 22 - 4:22 + 5] = np.nan
        cube_high_filtered[:, 17 - 4:17 + 5, 24 - 4:24 + 5] = np.nan
    planet_sep = int(
        np.sqrt((y0 - cube_high_filtered.shape[1] // 2) ** 2 + (x0 - cube_high_filtered.shape[2] // 2) ** 2))
    ...
    for i in range(len(lg)):
        for j in range(len(T)):
            print(round(100 * (k + 1) / (len(T) * len(lg)), 2), "%")
            CCF_rv, v_doppler, t = molecular_mapping_rv(instru, band, S_res, T[j], lg[i], model, wave, trans, "SNR_rv",
                                                        R=R, Rc=Rc, used_filter=used_filter, broad=broad)
    ...
    print(
        f"maximum SNR value of {round(np.nanmax(SNR_2d), 2)} for T = {T[idx_max_corr[1]]} K, lg = {lg[idx_max_corr[0]]} rv = {rv_2d[idx_max_corr[0], idx_max_corr[1]]} km/s")
    plt.figure()
    plt.pcolormesh(T, lg, SNR_2d, cmap=plt.get_cmap('rainbow'))
    plt.xlabel("planet's temperature (in K)", fontsize=12)
    plt.ylabel("planet's gravity surface", fontsize=12)
    plt.title(f'SNR value for different {model} spectra for {target_name} \n on {band} of {instru} with $R_c$ = {Rc}',
              fontsize=14)
    cbar = plt.colorbar()
    cbar.set_label("SNR", fontsize=14, labelpad=20, rotation=270)
    plt.plot([T[idx_max_corr[1]], T[idx_max_corr[1]]], [lg[idx_max_corr[0]], lg[idx_max_corr[0]]], 'kX', ms=10,
             label=r"$SNR_{max}$ = " + f"{round(np.nanmax(SNR_2d), 1)} for T = {T[idx_max_corr[1]]} K, \n lg = {lg[idx_max_corr[0]]} and rv = {round(rv_2d[idx_max_corr[0], idx_max_corr[1]], 2)} km/s")
    plt.legend()
    plt.show()
    plt.figure()
    plt.pcolormesh(T, lg, noise_2d, cmap=plt.get_cmap('rainbow'))
    plt.xlabel("planet's temperature (in K)", fontsize=12)
    plt.ylabel("planet's gravity surface", fontsize=12)
    plt.title(f'Noise value for different {model} spectra for {target_name} \n on {band} of {instru} with $R_c$ = {Rc}',
              fontsize=14)
    cbar = plt.colorbar()
    cbar.set_label("noise (in e-)", fontsize=14, labelpad=20, rotation=270)
    plt.show()
    plt.figure()
    plt.pcolormesh(T, lg, signal_2d, cmap=plt.get_cmap('rainbow'), vmin=np.nanmin(signal_2d), vmax=np.nanmax(signal_2d))
    plt.xlabel("planet's temperature (in K)", fontsize=12)
    plt.ylabel("planet's gravity surface", fontsize=12)
    plt.title(
        f'Signal value for different {model} spectra for {target_name} \n on {band} of {instru} with $R_c$ = {Rc}',
        fontsize=14)
    cbar = plt.colorbar()
    cbar.set_label("signal (in e-)", fontsize=14, labelpad=20, rotation=270)
    plt.show()
...
def chi_2_rv(instru, sd_HF, wave, trans, T, lg, sigma, sigma_l, model="BT-Settl", show=True, large_rv=False, broad=0):
    ...
    if show:
        plt.figure()
        plt.plot(rv, cos_theta, 'k')
        plt.xlabel("Doppler velocity (in km/s)", fontsize=14)
        plt.ylabel(r"cos($\theta$)", fontsize=14)
        plt.grid(True)
        plt.figure()
        plt.plot(wave, planet_spectrum_model.high_pass_flux, 'r')
        plt.plot(wave, sd_HF, 'b')
    return chi2, rv
...
def chi2_T_rv(instru, sd_HF, wave, trans, sigma_l, sigma, target_name, band, Rc, model="BT-Settl", broad=0):
    ...
    for i in range(len(lg)):
        for j in range(len(T)):
            print(round(100 * (k + 1) / (len(T) * len(lg)), 2), "%")
            chi2_rv, rv = chi_2_rv(instru, sd_HF, wave, trans, T=T[j], lg=lg[i], sigma=sigma, sigma_l=sigma_l,
                                   model=model, show=False, large_rv=False, broad=broad)
            chi2_2d[i, j] = np.nanmin(chi2_rv)
            rv_2d[i, j] = rv[chi2_rv.argmin()]
            k += 1
    idx_min_chi2 = np.unravel_index(np.argmin(chi2_2d, axis=None), chi2_2d.shape)
    chi2_min = np.nanmin(chi2_2d)
    chi2_2d = chi2_2d - chi2_min
    print(
        f"minimum chi2 value is given for T = {T[idx_min_chi2[1]]} K, lg = {lg[idx_min_chi2[0]]} rv = {rv_2d[idx_min_chi2[0], idx_min_chi2[1]]} km/s")
    plt.figure()
    plt.pcolormesh(T, lg, chi2_2d, cmap=plt.get_cmap('rainbow_r'))
    plt.xlabel("planet's temperature (in K)", fontsize=12)
    plt.ylabel("planet's gravity surface", fontsize=12)
    plt.title(
        r'$\chi^2$' + f' between {model} spectra and {target_name} \n data spectrum on {band} of {instru} with $R_c$ = {Rc}',
        fontsize=14)
    cbar = plt.colorbar()
    cbar.set_label(r"$\chi^2$ - $\chi^2_{min}$", fontsize=14, labelpad=20, rotation=270)
    plt.plot([T[idx_min_chi2[1]], T[idx_min_chi2[1]]], [lg[idx_min_chi2[0]], lg[idx_min_chi2[0]]], 'kX', ms=10,
             label=r"$\chi^2_{min}$ " + f"for T = {T[idx_min_chi2[1]]} K, \n lg = {lg[idx_min_chi2[0]]} and rv = {round(rv_2d[idx_min_chi2[0], idx_min_chi2[1]], 2)} km/s")
    plt.legend()
    plt.show()
