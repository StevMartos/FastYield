...
class Spectrum:
    ...
    def broad(self, vsini, epsilon=0.8, fastbroad=True):
        ...
        if fastbroad :
            ...
            flux = pyasl.rotBroad(self.wavelength[~np.isnan(self.flux)]*1e4, self.flux[~np.isnan(self.flux)], epsilon=epsilon, vsini=vsini) # 
            f = interp1d(self.wavelength[~np.isnan(self.flux)], flux, bounds_error=False, fill_value=np.nan) # créer une interpolation avec l'axe lambda décalé / le spectre décalé
...
def load_planet_spectrum(T, lg, model="BT-Settl", instru=None, load_path=load_path):
    ...
    dwl = wave - np.roll(wave, 1) ; dwl[0] = dwl[1] ; dwl[dwl==0] = np.nanmean(dwl) # array de delta Lambda
...
def transmission(instru,wave_inter,band,tellurics,apodizer):
    ...
    if instru == "MIRIMRS" or instru == "NIRSpec":
        trans *= fits.getheader("sim_data/PSF/PSF_"+instru+"/PSF_"+band+"_NO_JQ_NO_SP.fits")['AC'] # aperture corrective factor
...
def PSF_profile_fraction_separation(band,strehl,apodizer,coronagraph,instru,config_data,sep_unit):
    ...
    fraction_PSF = fits.getheader(file)['FC'] # fraction du flux dans une boîte de size_core²
...
def systematic_profile(config_data,band,trans,Rc,R,star_spectrum_instru,planet_spectrum,wave_inter,size_core,used_filter,show_cos_theta_est=False,PCA=False,Nc=100,mag_planet=None,band0=None,separation_planet=None):
    pca = None
    ...
    if instru=="MIRIMRS" :
        file="data/MIRIMRS/MIRISim/star_center/star_center_T"+str(T_star_sim)+"K_mag7_s3d_"+band+"_"+correction ; sigma_cosmic = None
    ...
    try :
        if data : 
            S_noiseless = fits.getdata("sim_data/Systematics/"+instru+"/S_data_star_center_s3d_"+band+"_"+correction+".fits") # in e-/mn
            pxscale = fits.getheader("sim_data/Systematics/"+instru+"/S_data_star_center_s3d_"+band+"_"+correction+".fits")['pxscale']
            wave = fits.getdata("sim_data/Systematics/"+instru+"/wave_data_star_center_s3d_"+band+"_"+correction+".fits")
        else :
            S_noiseless = fits.getdata("sim_data/Systematics/"+instru+"/S_noiseless_star_center_T"+str(T_star_sim)+"K_mag7_s3d_"+band+"_"+correction+".fits") # in e-/mn
            pxscale = fits.getheader("sim_data/Systematics/"+instru+"/S_noiseless_star_center_T"+str(T_star_sim)+"K_mag7_s3d_"+band+"_"+correction+".fits")['pxscale']
            wave = fits.getdata("sim_data/Systematics/"+instru+"/wave_noiseless_star_center_T"+str(T_star_sim)+"K_mag7_s3d_"+band+"_"+correction+".fits")
    except :
        file += ".fits"
        ...
        if data :
            fits.writeto("sim_data/Systematics/"+instru+"/S_data_star_center_s3d_"+band+"_"+correction+".fits",S_noiseless,header=hdr,overwrite=True)
            fits.writeto("sim_data/Systematics/"+instru+"/wave_data_star_center_s3d_"+band+"_"+correction+".fits",wave,overwrite=True)
        else :
    ...
    y_center = NbLine//2 ; x_center = NbColumn//2
    star_flux_FC = star_spectrum_instru.degrade_resolution(wave,renorm=True).flux * trans
    total_flux = np.nansum(star_flux_FC) # en e-/mn
    sep = np.zeros((int(round(config_data["spec"]["FOV"]/(2*pxscale)))+1))
    sigma_syst_prime_2 = np.zeros_like(sep)
    M_HF = np.zeros((len(sep),len(wave_inter))) # array 2D de taille 2x Nbline (ou Column) /2
    star_flux_data = np.nansum(S_noiseless,(1,2)) # m_gamma_S* en e-/mn du cube simulé sans bruits
    ...
    if instru=="MIRIMRS" and not data and 1==1 : 
        ...
        star_flux_data = input_flux.flux*np.nanmean(star_flux_data)/np.nanmean(input_flux.flux) # S_star
    
    cube_wo_planet = np.zeros_like(S_noiseless)+np.nan
    for i in range(NbChannel):
        cube_wo_planet[i] = S_noiseless[i]/star_flux_data[i] * star_flux_FC[i] # M_m_S
        
    S_res_wo_planet, _ = stellar_high_filtering(cube_wo_planet,renorm_cube_res=False,R=R,Rc=Rc,used_filter=used_filter,cosmic=data,sigma_cosmic=sigma_cosmic,print_value=False)
        
    if PCA and Nc!=0 :
        if mag_planet is None or separation_planet is None:
            raise KeyError("need a mag for the planet")
        ...
        planet_HF,planet_BF = filtered_flux(planet_flux_FC/trans,R=R,Rc=Rc,used_filter=used_filter)
        star_HF,star_BF = filtered_flux(star_flux_FC/trans,R=R,Rc=Rc,used_filter=used_filter)
        
        y0 = int(round(NbColumn/2+separation_planet/pxscale)) ; x0 = int(round(NbLine/2))
        shift = int(round(min(separation_planet,config_data["spec"]["FOV"]/2-pxscale)/pxscale))
        cube_shift = np.roll(np.copy(cube_wo_planet)*annular_mask(0,int(round(config_data["spec"]["FOV"]/4/pxscale)),size=(NbLine,NbColumn)),shift,1)
        for i in range(NbChannel):
            cube_shift[i] *= planet_flux_FC[i]/star_flux_FC[i]
        cube = np.copy(cube_wo_planet) + np.nan_to_num(cube_shift)
        
        S_res, _ = stellar_high_filtering(cube,renorm_cube_res=False,R=R,Rc=Rc,used_filter=used_filter,cosmic=data,sigma_cosmic=sigma_cosmic,print_value=False)

        mask = np.copy(S_res) * annular_mask(0,int(round(config_data["spec"]["FOV"]/2/pxscale)),size=(NbLine,NbColumn)) ; mask[~np.isnan(mask)] = 0
        S_res_pca, pca = PCA_subtraction(np.copy(S_res), n_comp_sub=Nc, y0=y0, x0=x0, size_core=size_core, PCA_annular=False, scree_plot=False, PCA_mask=True, PCA_plots=False, wave=wave, R=R)
        S_res_pca += mask
        
        
        Sres_wo_planet = np.copy(S_res)
        Sres_wo_planet[:,y0-size_core:y0+size_core+1,x0-size_core:x0+size_core+1] = np.nan
        Sres_wo_planet_pca = np.copy(S_res_pca)
        Sres_wo_planet_pca[:,y0-size_core:y0+size_core+1,x0-size_core:x0+size_core+1] = np.nan

        
        
        s_res = np.zeros_like(S_res)+np.nan 
        s_res_pca = np.zeros_like(S_res_pca)+np.nan 
        ...
        for i in range(NbLine):
            for j in range(NbColumn): # pour chaque spaxel
                ...
                s_res_pca[:,i,j] = np.nansum(S_res_pca[:,i-size_core//2:i+size_core//2+1,j-size_core//2:j+size_core//2+1],axis=(1,2))
        ...
        s_res_pca[s_res_pca==0] = np.nan ; S_res_pca = s_res_pca
        ...
        CCF, _ = molecular_mapping_rv(instru=instru,cube_high_filtered=S_res,T=planet_spectrum.T,lg=planet_spectrum.lg,model=planet_spectrum.model,wave=wave,trans=trans,R=R,Rc=Rc,used_filter=used_filter,rv=planet_spectrum.delta_rv,vsini=0,print_value=False,planet_spectrum=planet_spectrum,pca=None)
        CCF_pca, _ = molecular_mapping_rv(instru=instru,cube_high_filtered=S_res_pca,T=planet_spectrum.T,lg=planet_spectrum.lg,model=planet_spectrum.model,wave=wave,trans=trans,R=R,Rc=Rc,used_filter=used_filter,rv=planet_spectrum.delta_rv,vsini=0,print_value=False,planet_spectrum=planet_spectrum,pca=pca)
        CCF_wo_planet, _ = molecular_mapping_rv(instru=instru,cube_high_filtered=Sres_wo_planet,T=planet_spectrum.T,lg=planet_spectrum.lg,model=planet_spectrum.model,wave=wave,trans=trans,R=R,Rc=Rc,used_filter=used_filter,rv=planet_spectrum.delta_rv,vsini=0,print_value=False,planet_spectrum=planet_spectrum,pca=None)
        CCF_wo_planet_pca, _ = molecular_mapping_rv(instru=instru,cube_high_filtered=Sres_wo_planet_pca,T=planet_spectrum.T,lg=planet_spectrum.lg,model=planet_spectrum.model,wave=wave,trans=trans,R=R,Rc=Rc,used_filter=used_filter,rv=planet_spectrum.delta_rv,vsini=0,print_value=False,planet_spectrum=planet_spectrum,pca=pca)
        
        from src.molecular_mapping import SNR_calculation
        
        _, _, CCF_signal, CCF_noise = SNR_calculation(CCF,CCF_wo_planet,y0,x0,size_core,print_value=False,snr_calc=False)
        signal = CCF_signal-np.nanmean(CCF_noise)
        
        _, _, CCF_signal_pca, CCF_noise_pca = SNR_calculation(CCF_pca,CCF_wo_planet_pca,y0,x0,size_core,print_value=False,snr_calc=False)
        signal_pca = CCF_signal_pca-np.nanmean(CCF_noise_pca)
        
        M_pca = signal_pca / signal
        print(f"HERE : M_pca = {M_pca}")
        ...
        if Rc is None :
            ...
            d = trans*planet_HF - trans*star_HF*planet_BF/star_BF
        template = trans*planet_HF
        template /= np.sqrt(np.nansum(template**2))
    
        d_sub = np.reshape(np.copy(d), (NbChannel, 1 * 1)).transpose()
        d_sub[np.isnan(d_sub)] = 0
        X = pca.transform(d_sub)
        X = pca.inverse_transform(X)
        d_sub = (d_sub - X).transpose()
        d_sub = d_sub[:,0]
        d_sub[d_sub == 0] = np.nan

        template_sub = np.reshape(np.copy(template), (NbChannel, 1 * 1)).transpose()
        template_sub[np.isnan(template_sub)] = 0
        X = pca.transform(template_sub)
        X = pca.inverse_transform(X)
        template_sub = (template_sub - X).transpose()
        template_sub = template_sub[:,0]
        template_sub[template_sub == 0] = np.nan
        
        m_pca = np.nansum(d_sub*template_sub) / np.nansum(d*template)
        print(f"HERE : m_pca = {m_pca}")
        
        
        
        M_pca = min(abs(M_pca),abs(m_pca),1)
        
        mask = np.copy(S_res_wo_planet) * annular_mask(0,int(round(config_data["spec"]["FOV"]/2/pxscale)),size=(NbLine,NbColumn)) ; mask[~np.isnan(mask)] = 0
        S_res_wo_planet,pca = PCA_subtraction(S_res_wo_planet, n_comp_sub=Nc, size_core=size_core, wave=wave, R=R, scree_plot=False, PCA_plots=False)
        S_res_wo_planet += mask
    ...
    s_res_wo_planet = np.zeros_like(S_res_wo_planet)+np.nan 
    for i in range(NbLine):
        for j in range(NbColumn): # pour chaque spaxel
            s_res_wo_planet[:,i,j] = np.nansum(S_res_wo_planet[:,i-size_core//2:i+size_core//2+1,j-size_core//2:j+size_core//2+1],axis=(1,2))
    s_res_wo_planet[s_res_wo_planet==0] = np.nan ; S_res_wo_planet = s_res_wo_planet
    
    CCF_wo_planet, _ = molecular_mapping_rv(instru=instru,cube_high_filtered=S_res_wo_planet,T=planet_spectrum.T,lg=planet_spectrum.lg,model=planet_spectrum.model,wave=wave,trans=trans,R=R,Rc=Rc,used_filter=used_filter,rv=planet_spectrum.delta_rv,vsini=0,print_value=False,planet_spectrum=planet_spectrum,pca=pca)

    for r in range(1, len(sep)+1):
        sep[r-1] = r*pxscale
        ccf = np.copy(CCF_wo_planet)*annular_mask(max(1,r-1),r,size=(NbLine, NbColumn)) # anneau du cube M à la séparation r
        if not all(np.isnan(ccf.flatten())):
            ...
            if show_cos_theta_est :
                f = interp1d(wave, S_res_wo_planet[:,y_center+1-r,x_center+1-r]/star_flux_FC, bounds_error=False, fill_value=np.nan)
                M_HF[r-1,:] = f(wave_inter)
    
    Mp = np.nanmean(cube_wo_planet[:,y_center-size_core//2:y_center+size_core//2+1,x_center-size_core//2:x_center+size_core//2+1],axis=(1,2))/star_flux_FC # car on somme le signal sur la FWHM
    Mp /= np.nanmean(Mp)
    #Mp = fits.getdata("utils/Mp/Mp_"+band+"_"+str(planet_spectrum.T)+"K.fits")
