...
def log_interpolate(y_lo, y_hi, x_lo, x_hi, x):
    """
    Linear interpolation in log10(x).
    Useful for pressure grids sampled logarithmically.
    """
...
def transmission_to_tau(transmission, floor=1e-300):
    """
    Convert transmission to optical depth tau = -ln(T).
    """
    transmission = np.asarray(transmission, dtype=float)
    transmission = np.clip(transmission, floor, 1.0)
    return -np.log(transmission)



def tau_to_transmission(tau):
    """
    Convert optical depth tau to transmission T = exp(-tau).
    """
...
def get_logL(flux_observed, flux_model, sigma_l, method="classic"): # see https://github.com/exoAtmospheres/ForMoSA/blob/activ_dev/ForMoSA/nested_sampling/nested_logL_functions.py
    ...
    if method == "classic":  # true Gaussian log-likelihood with optimal scale R
        ...
        R      = np.sum(f * t / s2) / np.sum(t * t / s2)
        chi2   = np.sum(((f - R * t) ** 2) / s2)
        s2_hat = chi2 / N
        logL   = -(chi2 / (2.0 * s2_hat) + (N / 2.0) * np.log(2.0 * np.pi * s2_hat) + 0.5 * np.log(np.sum(s2)))

    elif method == "extended_bis":
        f_n    = f / np.sqrt(np.sum((f * f) / s2) + eps)
        g_n    = t / np.sqrt(np.sum((t * t) / s2) + eps)
        chi2   = np.sum(((f_n - g_n) ** 2) / s2)
        s2_hat = chi2 / N
        logL   = -(chi2 / (2.0 * s2_hat) + (N / 2.0) * np.log(2.0 * np.pi * s2_hat) + 0.5 * np.log(np.sum(s2)))

    elif method == "Brogi":
        f_n  = f / np.sqrt(np.sum(f * f) + eps)
        g_n  = t / np.sqrt(np.sum(t * t) + eps)
        Sf2  = np.sum(f_n * f_n) / N
        Sg2  = np.sum(g_n * g_n) / N
        R    = np.sum(f_n * g_n) / N
        ...
        sigma2_weight = 1.0 / (np.sum(1.0 / (s2 + eps)) / N + eps)
