...
def get_DIT_RON(config_data, instru_type, apodizer, PSF_profile, separation, star_spectrum_band, exposure_time, min_DIT, max_DIT, trans, RON0, RON_lim, saturation_e, input_DIT, separation_planet=None):
    ...
    if "fiber_injection_HRS" in instru_type and DIT_saturation_planet is not None:
    ...
    # Up-the-ramp effective readout-noise in [e-/px/DIT] (see https://arxiv.org/pdf/0706.2344)
    if DIT >= 2*min_DIT: # At least 2 reads inside the ramp
        N_read  = DIT / min_DIT                                         # Number of intermittent non-destructive readings
        RON_eff = estimate_RON_UTR(N=N_read, RON0=RON0, RON_lim=RON_lim) # Effective read out noise [e-/px/DIT]
        RON_eff = min(RON_eff, RON0)                                     # [e-/px/DIT]
    else:
        RON_eff = RON0 # [e-/px/DIT]
    
    # Total number of integrations
    N_DIT = exposure_time / DIT
            
    return N_DIT, DIT, DIT_saturation, DIT_saturation_planet, RON_eff, iwa_FPM
...
def get_alpha_cos_theta_syst(Mp_Sp_HF, trans, template, R, Rc, filter_type):
    """
    ...
    ----------
    Mp_Sp_HF: 1d array
        HPF planet spectrum on the band grid [ph/bin/DIT] (total ph over the FoV).
    trans: 1d array
        Total system transmission on the same grid as the spectrum (scalar or array).
    template: 1d array
        (Unitless) normalized template on the same spectral grid.
...
def get_beta(Ss_HF, Ss_LF, Mp_Sp_LF, trans, template, R, Rc, filter_type):
    """
...
def get_systematics(config_data, band, tellurics, apodizer, strehl, coronagraph, R_band, Rc, filter_type, star_spectrum_instru, planet_spectrum_instru, stellar_component, wave_band, size_core, PCA=False, PCA_mask=False, N_PCA=20, mag_planet=None, separation_planet=None, mag_star=None, exposure_time=None, target_name=None, on_sky_data=False, sigma_outliers=3):
    """
    ...
    # Or create them (but the raw noiseless or on-sky data are needed)    
        ...
        trans_Ss_data[trans_Ss_data == 0] = np.nan
        
        # Estimating the stellar modulation function
        Ms = S_noiseless / (trans_Ss_data)[:, None, None]
    ...
    R_nyquist                   = get_resolution(wavelength=wave_data, func=np.nanmedian)
    ...
    # ---------------------------------------------------
    # 3) PCA branch (with optional fake-planet injection)
    # ---------------------------------------------------
    if PCA_calc: 
        ...
        if (mag_planet is not None) and (separation_planet is not None) and (separation_planet < FOV / 2):
            ...
            trans_Ss                = np.nansum(S, (1, 2)) # [e-/bin/mn] (total e- over the FoV)
            trans_Ss[trans_Ss == 0] = np.nan               # [e-/bin/mn] (total e- over the FoV)
            
            # Stellar filtering + PCA in [e-/bin/px/mn]
            S_res, _       = get_S_res(wave=wave_data, S=S, Rc=Rc, filter_type=filter_type, trans_Ss=trans_Ss, outliers=on_sky_data, sigma_outliers=sigma_outliers, renorm_S_res=False, only_high_pass=False, debug=False) # stellar subtracted data with the fake planet injected
        ...
        # No injection: PCA on the filtered cube without planet
            ...
            # Stellar filtering
            S_res_wo_planet, _ = get_S_res(wave=wave_data, S=S_wo_planet, Rc=Rc, filter_type=filter_type, trans_Ss=trans_Ss, outliers=on_sky_data, sigma_outliers=sigma_outliers, renorm_S_res=False, only_high_pass=False, debug=False)
        ...
        d          = get_d_sim(instru=instru, d=None, wave=wave_data, trans=trans, R=R_band, Rc=Rc, filter_type=filter_type, model=None, T=None, lg=None, rv=None, vsini=None, epsilon=None, fastbroad=None, airmass=None, star_spectrum=None, wave_model=None, template=planet_spectrum_instru, degrade_resolution=True, stellar_component=stellar_component, trans_Ss=trans_Ss, pca=None, cut_fringes=False, Rmin=None, Rmax=None, target_name=None, renorm_d_sim=False, sigma_l=None, Mp=None, verbose=False)
        d_pca      = get_d_sim(instru=instru, d=None, wave=wave_data, trans=trans, R=R_band, Rc=Rc, filter_type=filter_type, model=None, T=None, lg=None, rv=None, vsini=None, epsilon=None, fastbroad=None, airmass=None, star_spectrum=None, wave_model=None, template=planet_spectrum_instru, degrade_resolution=True, stellar_component=stellar_component, trans_Ss=trans_Ss, pca=pca,  cut_fringes=False, Rmin=None, Rmax=None, target_name=None, renorm_d_sim=False, sigma_l=None, Mp=None, verbose=False)
...
def get_gaussian_C(wave, Lcorr):
    ...
    ----------
    wave : array_like
        One-dimensional wavelength grid in [µm].
...
def get_sigma_base_2(h, C):
    """
    Evaluate the quadratic form sigma_base_2 = h^T C h (projected variance in the CCF for unitary modulation) while ignoring invalid entries.
    ...
    hv         = h[valid]
    Cv         = C[np.ix_(valid, valid)]
    sigma_base_2 = np.einsum("i,ij,j->", hv, Cv, hv, optimize=True)
    
    return sigma_base_2



def _to_radians(angle, unit):
    """
    Convert an angular quantity to radians.

    Parameters
    ----------
    angle : array_like or float
        Angular quantity.
    unit : {"mas", "arcsec", "rad"}
    ...
    Returns
    -------
    angle_rad : ndarray or float
        Angle converted to radians.

    Raises
    ------
    ValueError
        If 'unit' is not one of {"mas", "arcsec", "rad"}.
    """
    if unit == "mas":
        return angle / (rad2arcsec * 1000.0)
    if unit == "arcsec":
        return angle / rad2arcsec
    if unit == "rad":
        return angle
    raise ValueError("unit must be 'mas', 'arcsec', or 'rad'.")
...
def get_Lcorr_speckle(wave, separation, D, sep_unit, eps=1e-15):
    ...
    where:
      - 'wave' is the wavelength in [µm],
      - 'D' is the telescope diameter in [m],
      - 'rho' is the angular separation in radians,
      - 1.029 is the Airy-pattern FWHM in units of lambda / D.

    Parameters
    ----------
    wave : array_like or float
        Wavelength grid in [µm]. Can be a scalar or a 1D array.
    separation : array_like or float
        Angular separation from the star. Can be a scalar or a 1D array.
    D : float
        Telescope diameter in [m].
    sep_unit : {"mas", "arcsec", "rad"}
        Unit of 'separation'.
    eps : float, optional
        Minimum separation used to avoid division by zero. Default is 1e-15.

    Returns
    -------
    L_speck : ndarray or float
        Spectral correlation length in [µm].

        The output shape follows standard broadcasting rules:
          - scalar if both inputs are scalars,
          - 1D if one input is scalar and the other is 1D,
          - 2D with shape (n_wave, n_sep) if both inputs are 1D.

    Notes
    -----
    No clipping to the total bandwidth is applied here. The asymptotic regime
    where the whole band becomes effectively fully correlated is handled
    explicitly in the quadratic-form evaluation routines.
    """
    separation_rad = _to_radians(separation, sep_unit)
    rho            = np.maximum(separation_rad, eps)

    L_speck = 1.029 / (2.0 * np.sqrt(np.log(2.0))) / (1e6 * D) * wave**2 / rho

    return L_speck



def get_Lcorr_aliasing_spatial(wave, separation, pxscale, sep_unit, eps=1e-15):
    """
    ...
    L_al_spat : ndarray or float
        Spatial-aliasing spectral correlation length in [µm].
    ...
    separation_rad = _to_radians(separation, sep_unit)
    pxscale_rad    = _to_radians(pxscale, sep_unit)
    rho            = np.maximum(separation_rad, eps)

    L_al_spat = pxscale_rad / (2.0 * np.sqrt(np.log(2.0))) * wave / rho
...
def get_Lcorr_aliasing_spectral(wave):
    ...
    This width is then converted into the Gaussian covariance length used in
    ...
    -------
    L_al_spec : ndarray
        One-dimensional array of shape (n_wave,) containing the local spectral
        aliasing correlation length in [µm].

    Notes
    -----
    The adopted conversion is

        L_al_spec = d_lambda / (2 * sqrt(ln 2)),

    where d_lambda is the local wavelength sampling step. This choice makes the
    Gaussian kernel FWHM approximately equal to the local spectral sampling.
    """
    L_al_spec = np.gradient(wave) / (2*np.sqrt(np.log(2)))
    return L_al_spec



def compress_h_for_sigma_base_2(wave, h, post_processing, D, pxscale, separation_ref, sep_unit, oversample=3.0, max_merge=128):
    """
    Compress the spectral grid used for sigma_base_2 by merging neighboring channels.

    The compression factor is chosen so that the coarse spectral sampling
    remains smaller than the minimum speckle correlation length divided by
    'oversample', evaluated at a reference separation.

    The spectral weights are summed, not averaged, because sigma_base_2 is a
    quadratic form over the spectral channels and the integrated weight per
    coarse bin must be conserved.

    Parameters
    ----------
    wave : array_like
        Native wavelength grid, shape (n_wave,), in [µm].
    h : array_like
        Spectral weights entering the DI quadratic form, shape (n_wave, n_sep).
    post_processing : str
        Post processing method, "DI" or "MM".
    D : float
        Telescope diameter in [m].
    pxscale : float
        pixel scale sampling in 'sep_unit'/px.
    separation_ref : float
        Reference separation at which the compression is defined.
    sep_unit : {"mas", "arcsec", "rad"}
        Unit of 'separation_ref'.
    oversample : float, optional
        Number of coarse bins per minimum correlation length.
        Typical values are between 2 and 4. Default is 3.0.
    ...
    -------
    wave_c : ndarray
        Compressed wavelength grid, shape (n_wave_c,), in [µm].
    h_c : ndarray
        Compressed weights, shape (n_wave_c, n_sep).
    n_merge : int
        Number of native channels merged into one coarse channel.

    Notes
    -----
    This compression is purely numerical. It is meant to speed up the
    ...
    # Reference speckle correlation length on the native grid.
    dlam_native = np.nanmedian(np.diff(wave))

    # Reference speckle correlation length on the native grid.
    if post_processing == "DI":
        L_ref = np.nanmin(get_Lcorr_speckle(wave=wave, separation=separation_ref, D=D, sep_unit=sep_unit))
    elif post_processing == "MM":
        L_ref = np.nanmin(get_Lcorr_aliasing_spatial(wave=wave, separation=separation_ref, pxscale=pxscale, sep_unit=sep_unit))

    # Number of native bins merged into one coarse bin.
    dlam_target = L_ref / oversample
    n_merge     = int(np.floor(dlam_target / dlam_native))
    n_merge     = max(1, min(max_merge, n_merge))

    idx0   = np.arange(0, wave.size, n_merge)
    counts = np.diff(np.append(idx0, wave.size))
...
def compute_sigma_base_2_speck_numba(h, wave, separation_rad, D, n_sigma_cut=4.0, eps=1e-15):
    """
    Compute the speckle covariance quadratic form for all separations.

    For each separation, this function evaluates

        sigma_base_2 = h.T @ C_speck @ h,
    ...
    wave : ndarray
        One-dimensional wavelength grid of shape (n_lambda,), in [µm].
    ...
    n_sigma_cut : float, optional
        ...
        local correlation length. Contributions beyond this threshold are
        neglected. Default is 4.0.
    eps : float, optional
    ...
    -------
    out : ndarray
        One-dimensional array of shape (n_sep,) containing sigma_base_2 at each
        separation.

    Notes
    -----
    The local speckle correlation length is

        L_speck = [1.029 / (2 * sqrt(ln 2))] * wave^2 / (D * rho).

    When the minimum correlation length across the band becomes much larger than
    the total bandwidth, the covariance matrix tends to C_ij = 1 for all pairs.
    In that limit, the quadratic form is evaluated exactly as
...
@njit(cache=True, fastmath=True, parallel=True)
def compute_sigma_base_2_al_spat_numba(h, wave, separation_rad, pxscale_rad, n_sigma_cut=4.0, eps=1e-15):
    """
    Compute the spatial-aliasing quadratic form for all separations.

    For each separation, this function evaluates

        sigma_base_2 = h.T @ C_al_spat @ h,
    ...
    -------
    out : ndarray
        One-dimensional array of shape (n_sep,) containing sigma_base_2 for
        each separation.

    Notes
    -----
    The local spatial-aliasing correlation length is

        L_al_spat = pxscale_rad * wave / [2 * sqrt(ln 2) * rho].

    When the whole band becomes effectively fully correlated, the exact limit
...
@njit(cache=True, fastmath=True, parallel=True)
def compute_sigma_base_2_al_spec_numba(h, wave, n_sigma_cut=4.0):
    """
    ...
    where C_al_spec is the Gaussian covariance matrix associated with spectral
    aliasing.
    ...
    Notes
    -----
    The local spectral-aliasing correlation length is approximated as

        L_i = d_lambda_i / (2 * sqrt(ln 2)),

    where d_lambda_i is the local native wavelength sampling step.

    As in the speckle case, when the whole spectral band becomes effectively
    fully correlated, the exact limit
    ...
    for i in range(nlam):
        ...
        L[i] = dlam / (2*np.sqrt(np.log(2)))

    Dwave = wave[nlam - 1] - wave[0]

    # Smallest local correlation length on the band.
    Lmin_global = L[0]
    for i in range(1, nlam):
        if L[i] < Lmin_global:
            Lmin_global = L[i]

    for k in prange(nsep):
        # Fully correlated limit across the whole spectral band.
        if Lmin_global > n_sigma_cut * Dwave:
            s = 0.0
            for i in range(nlam):
                hi = h[i, k]
                if np.isfinite(hi) and hi != 0.0:
